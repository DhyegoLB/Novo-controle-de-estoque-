...
namespace controle_de_estoque1
{
    public partial class FrmCDfuncionario : Form
    {
            ...
            try
            {
                using (conexao = new MySqlConnection(data_source))
                {
                    ...
                    using (MySqlCommand comando = new MySqlCommand(sql, conexao))
                    {
                        ...
                        // GERANDO O COMANDO DA TABELA DE CADASTRO DO FUNCINARIO
        ...
        private void butAtualizar_Click(object sender, EventArgs e) //ATUALIZAR O CADASTRO
        {
            try
            {
                using (conexao = new MySqlConnection(data_source))
                {
                    string sql = "UPDATE funcionario SET senha = @senha, nome = @nome,cpf = @cpf, contato = @contato, rg = @rg, bairro = @bairro, email = @email, cep = @cep, pis = @pis, carteira_trabalho = @carteira_trabalho, genero = @genero, estado_civil = @estado_civil, uf = @uf, nacionalidade = @nacionalidade WHERE id = @id";
                    ...
                    using (MySqlCommand comando = new MySqlCommand(sql, conexao))
                    {
                        ...
                        if (!ValidarSenha(senha))
                        {
                            ...
                            MessageBox.Show("Pelo menos 8 caracteres de comprimento.\nPelo menos uma letra maiúscula (A-Z).\nPelo menos uma letra minúscula (a-z).\nPelo menos um dígito numérico (1-9).\nPelo menos um caractere especial (!, @, #, $, %, etc.).\nNão contém caracteres repetidos consecutivos.");
                            return;
                        }
        ...
        private void butDesativar_Click(object sender, EventArgs e)
        {
           // Caso precise
        }
    }
